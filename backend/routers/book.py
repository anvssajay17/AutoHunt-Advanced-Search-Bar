...
def search_word(q: str = Query(..., min_length=1), db: Session = Depends(get_db)):
    ...
    query = text("""
        SELECT
            name,
            (
                CASE WHEN position(:q in name) > 0 THEN 40 ELSE 0 END +
                CASE WHEN similarity(name, :q) > 0.1 THEN similarity(name, :q) * 50 ELSE 0 END +
                CASE WHEN metaphone = :phonetic THEN 10 ELSE 0 END
            ) AS score
        FROM books
        WHERE
    ...
    result = db.execute(query, {"q": q, "phonetic": q_metaphone}).fetchone()
